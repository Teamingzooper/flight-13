...
import { customAbility, hasAbility, usesLeft } from './custom';
...
import type { CustomAbility, GameState, MoveTarget, NightAction, PlayerState, RoleId, SeatId } from './types';
...
export function checkAction(s: GameState, p: PlayerState, action: NightAction): string | null {
  ...
  if (isGuest(s, p.id)) {
    ...
    if (action?.kind === 'treat' && hasAbility(s.settings, p.role, 'treat') && usesLeft(s, p) !== 0 && pilot && action.target === pilot.id) return null;
    if (action?.kind === 'knockout' && isSaboteur(p.role) && pilot) return null;
    return 'You are on the flight deck tonight.';
  }
  // A custom role's ability, once its uses for the flight are gone.
  const spent = 'You have used up your ability for this flight.';
  switch (action?.kind) {
    case 'treat': {
      if (!hasAbility(s.settings, p.role, 'treat')) return 'Only the Nurse can treat people.';
      if (usesLeft(s, p) === 0) return spent;
    ...
    case 'sweep':
      if (!hasAbility(s.settings, p.role, 'sweep')) return 'Only the Investigator can sweep for bombs.';
      return usesLeft(s, p) === 0 ? spent : null;
    case 'inspect': {
      if (!hasAbility(s.settings, p.role, 'sweep')) return 'Only the Investigator can inspect.';
      if (usesLeft(s, p) === 0) return spent;
      if (action.what === 'cart') {
    ...
    case 'poison': {
      if (!hasAbility(s.settings, p.role, 'poison')) return 'You have no poison.';
      if (usesLeft(s, p) === 0) return spent;
      const t = reachable(s, p, action.target);
      if (typeof t === 'string') return t;
      if (t.id === p.id) return 'You cannot poison yourself.';
      if (onFlightDeck(s, t)) return `${t.name} is on the flight deck.`;
      if (distance(cellOf(p), cellOf(t)) > 1) return `${t.name} is too far away. Sit next to them first.`;
      return null;
    }
    case 'plant': {
      if (!hasAbility(s.settings, p.role, 'bomb')) return 'You have no bomb.';
    ...
    case 'cuff': {
      if (!hasAbility(s.settings, p.role, 'cuff')) return 'Only the Air Marshal carries handcuffs.';
      const left = usesLeft(s, p);
      if (left === null ? p.cuffsUsed : left === 0) return 'You already used your handcuffs.';
      const t = reachable(s, p, action.target);
      if (typeof t === 'string') return t;
...
/** The built-in role whose moves a custom ability uses (poison has its own). */
const CUSTOM_AS: Record<Exclude<CustomAbility, 'none'>, RoleId | 'poison'> = {
  treat: 'nurse',
  sweep: 'investigator',
  cuff: 'marshal',
  bomb: 'bomber',
...
export function possibleActions(s: GameState, p: PlayerState): NightAction[] {
  ...
  // A custom role acts like the built-in role it borrowed its ability from.
  const custom = customAbility(s.settings, p.role);
  const as = custom === null ? p.role : custom === 'none' ? 'passenger' : CUSTOM_AS[custom];
  switch (as) {
    case 'poison':
      for (const t of everyone) candidates.push({ kind: 'poison', target: t.id });
      break;
    case 'nurse':
